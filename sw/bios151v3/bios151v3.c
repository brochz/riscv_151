...
int8_t* read_token(int8_t* b, uint32_t n, int8_t* ds)
{
    uint32_t i = 0;
    while (i < n) {
        int8_t ch = uread_int8();    //This function will wait for a char
        if (ch == '\x08') {          //x08 stands for backspace
            if (i == 0)              //jmp出了问题?????
                uwrite_int8('\x20'); //Space
            else {
                b[i] = '\0';         //调整b[]的长度
                i = i - 1;      
                uwrite_int8s("\x20\x08"); //write space & backspace ????
            }
        } 
        else {
            for (uint32_t j = 0; ds[j] != '\0'; j++) {
                if (ch == ds[j]) {      //ch等于ds中的任何一个字符串就返回
                    b[i] = '\0';        //字符串中不包含ds
                    return b;           //字符串在这里返回
                }
            }
...
void store(uint32_t address, uint32_t length)
{
    for (uint32_t i = 0; i*4 < length; i++) {
        int8_t buffer[9];
        int8_t* ascii_instruction = read_n(buffer,8);  //read 8 char
...
int main(void)
{
    ...
    for ( ; ; ) {
        ...
        //*input = *buffer
        int8_t* input = read_token(buffer, BUFFER_LEN, " \x0d"); //0d stands for cr
