...
int main(int argc, char**argv) {
  ...
  dma_read_ddr(WT_CONV1_DDR_ADDR,
               (uint32_t)wt_conv1 >> 2,    //一次取32bit数据ok
               WT_CONV1_SIZE >> 2);
  // Load wt_conv2
  ...
  for (i = 0; i < NUM_TEST_IMAGES; i++) {
    ...
    // Write the OFM result to DDR at address 0x00003600
    conv3D_hw(IMAGES_DDR_ADDR + i * IMG_SIZE, WT_CONV1_DDR_ADDR, 0x00003600,
              IMG_DIM, IMG_DEPTH, CV1_DIM, CV1_DEPTH);

    // Read the OFM result (computed by the accelerator) to the
    // local conv1_ofm in RISC-V DMem
    dma_read_ddr(0x00003600, (uint32_t)conv1_ofm >> 2, CONV1_OFM_SIZE);
    ...
    dma_write_ddr((uint32_t)pool1_ofm >> 2, 0x00003600, POOL1_OFM_SIZE >> 2);

    // Perform conv3D on the accelerator
    // Read IFM from DDR 0x00003600
    // Write the OFM result to 0x00007E00
    conv3D_hw(0x00003600, WT_CONV2_DDR_ADDR, 0x00007E00,
              P1_DIM, P1_DEPTH, CV2_DIM, CV2_DEPTH);

    // Read the OFM result (computed by the accelerator) to the
    // local conv2_ofm in RISC-V DMem
    dma_read_ddr(0x00007E00, (uint32_t)conv2_ofm >> 2, CONV2_OFM_SIZE);
